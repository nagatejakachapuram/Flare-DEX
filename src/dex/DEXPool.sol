...
contract DEXPool is ReentrancyGuard {
    ...
    /// @notice Returns the current reserves of token0 in the pool
    /// @return Reserve of token0
    function getReserve0() external view returns (uint256) {
        return reserve0;
    }

    /// @notice Returns the current reserves of token1 in the pool
    /// @return Reserve of token1
    function getReserve1() external view returns (uint256) {
        return reserve1;
    }

    /// @notice Returns the current active tick of the pool
    /// @return Current active tick
    function getCurrentTick() external view returns (int24) {
        return currentTick;
    }

    /// @notice Returns the current liquidity of the pool
    /// @return Liquidity in the pool
    function getLiquidity() external view returns (uint128) {
        return liquidity;
    }

    /// @notice Returns the current sqrt price (Q64.96) of the pool
    /// @return Current sqrt price of the pool
    function getSqrtPriceX96() external view returns (uint160) {
        return sqrtPriceX96;
    }

    /// @notice Returns the liquidity gross at a specific tick
    /// @param tick Tick index
    /// @return Liquidity gross at the given tick
    function getLiquidityGross(int24 tick) external view returns (uint128) {
        return ticks[tick].liquidityGross;
    }

    /// @notice Returns the liquidity net at a specific tick
    /// @param tick Tick index
    /// @return Liquidity net at the given tick
    function getLiquidityNet(int24 tick) external view returns (int128) {
        return ticks[tick].liquidityNet;
    }

    /// @notice Returns the FTSO oracle price for a token (token0 or token1)
    /// @param tokenIndex Index of token (0 for token0, 1 for token1)
    /// @return price Price of the token from the FTSO oracle
    function getFTSOPrice(
        uint8 tokenIndex
    ) external view returns (uint256 price) {
        (price, ) = ftsoOracle.fetchPrice(tokenIndex);
    }

    /// @notice Returns the price pair symbol for the trading pair
    /// @return The price pair symbol (e.g., "WC2FLR/C2FLR")
    function getPricePairSymbol() external view returns (string memory) {
        return pricePairSymbol;
    }
}
